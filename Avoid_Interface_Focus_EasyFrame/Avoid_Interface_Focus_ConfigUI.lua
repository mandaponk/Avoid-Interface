-- Avoid_Interface_focus_ConfigUI.lua
-- Baut die "focus"-Seite im gemeinsamen Avoid Interface Config-Fenster
AI       = AI or {}
AI.ConfigUI = AI.ConfigUI or {}

-- Stellt sicher, dass AI_Config.modules[moduleKey] existiert
local function GetfocusEntry(moduleKey)
...
local function GetfocusModule()
    if AI and AI.modules and type(AI.modules.focus) == "table" then
        return AI.modules.focus
    end
end

local function ApplyfocusLayout()
    local m = GetfocusModule()
    if m and m.ApplyLayout then
        m.ApplyLayout()
    elseif AI and AI.RefreshModule then
        AI.RefreshModule("focus")
    end
end


local function BuildfocusConfigPage(page, moduleKey, labelText, helpers)
    ...
    -------------------------------------------------
    -- Move- und Reset-Buttons (nur focus-Frame)
    -------------------------------------------------
    ...
    moveButton:SetScript("OnClick", function(self)
        ...
        -- wie beim focus: sobald der Move-Mode umgeschaltet wird,
    ...
    resetPosButton:SetScript("OnClick", function()
        ...
        -- Layout neu anwenden (focus, nicht focus)
    ...
    local debuffsEnableCheck, debuffsSizeSlider, debuffsMaxSlider, debuffsPerRowSlider, debuffsAnchorDD, debuffsXSlider, debuffsYSlider, debuffsGrowDD
        ...
        -- BorderSize je nach Stil aktiv/deaktivieren (für focus)
        local function UpdateBorderSizeSliderEnabled(style)
            local cfg = GetfocusEntry(moduleKey)
        ...
        leaderIconXSlider = CreateSliderWithInput(
            "AI_Focus_LeaderIconXOffsetSlider",
            page,
        ...
        leaderIconYSlider = CreateSliderWithInput(
            "AI_Focus_LeaderIconYOffsetSlider",
            page,
        ...
        raidIconCheck.text:SetText("Raidfocus-Icon anzeigen")

        raidIconSizeSlider = CreateSliderWithInput(
            "AI_Focus_RaidIconSizeSlider",
            page,
            "Raidfocus-Icon Größe",
            8, 64, 1,
            raidIconCheck,
            -24
        )
        ...
        raidIconXSlider = CreateSliderWithInput(
            "AI_Focus_RaidIconXOffsetSlider",
            page,
            "Raidfocus-Icon X-Offset",
            -200, 200, 1,
            raidIconSizeSlider,
            -24
        )
        ...
        raidIconYSlider = CreateSliderWithInput(
            "AI_Focus_RaidIconYOffsetSlider",
            page,
            "Raidfocus-Icon Y-Offset",
            -200, 200, 1,
            raidIconXSlider,
            -24
        )
        ...
        raidIconAnchorDD = CreateSimpleDropdown(
            "AI_Focus_RaidIconAnchorDD",
            page,
            "Raidfocus-Icon Anker",
            anchorItems,
            raidIconYSlider,
            -24
        )
        ...
        -------------------------------------------------
        -- focus-Controls am Page-Objekt speichern
        -------------------------------------------------
    ...
    -------------------------------------------------
   page.Init = function()
    local entry = GetfocusEntry(moduleKey)
    ...
    elseif entry.enabled == nil then
        ...
        check:SetChecked(entry.enabled and true or false)
            ...
            -- Icon-Defaults (nur für Config-UI, eigentliche Logik ist im focus-Modul)
    ...
    check:SetScript("OnClick", function(self)
        ...
        -- Buff-Options (focus, nicht focus!)
    ...
    if moveButton then
        moveButton:SetScript("OnClick", function(self)
            ...
            if cfg.movable then
                self:SetText("Beenden")
                if AI and AI.modules and AI.modules.focus and AI.modules.focus.StartMovingMode then
                    AI.modules.focus.StartMovingMode()
                end
            else
                self:SetText("Frame bewegen")
                if AI and AI.modules and AI.modules.focus and AI.modules.focus.StopMovingMode then
                    AI.modules.focus.StopMovingMode()
                end
            end
    ...
    if resetPosButton then
        resetPosButton:SetScript("OnClick", function(self)
            AI_Config.modules = AI_Config.modules or {}
            AI_Config.modules.focus = AI_Config.modules.focus or {}
            local cfg = AI_Config.modules.focus

            -- Frame-Position im Modul auf Default zurücksetzen
            if AI and AI.modules and AI.modules.focus and AI.modules.focus.ResetPosition then
                AI.modules.focus.ResetPosition()
            else
            ...
            -- UI neu initialisieren (setzt die Defaults aus GetfocusConfig/page.Init)
            if page.Init then
                page.Init()
            end
...
if AI.ConfigUI and AI.ConfigUI.RegisterPage then
    AI.ConfigUI.RegisterPage("focus", "focus", 30, BuildfocusConfigPage)
end