-- Avoid_Interface_target_ConfigUI.lua
-- Baut die "target"-Seite im gemeinsamen Avoid Interface Config-Fenster
AI       = AI or {}
...
local function GetTargetModule()
    if AI and AI.modules and type(AI.modules.target) == "table" then
        return AI.modules.target
    end
end

local function ApplyTargetLayout()
    local m = GetTargetModule()
    if m and m.ApplyLayout then
        m.ApplyLayout()
    elseif AI and AI.RefreshModule then
        AI.RefreshModule("target")
    end
end


local function BuildTargetConfigPage(page, moduleKey, labelText, helpers)
    ...
    do
        ...
        presetDropdown = CreateSimpleDropdown(
            "AI_Target_PresetDD",   -- Name (STRING!)
            page,                   -- Parent
            "Preset",               -- Label
            presetItems,            -- Items
    ...
    -------------------------------------------------
    -- Move- und Reset-Buttons (nur target-Frame)
    -------------------------------------------------
    ...
    moveButton:SetScript("OnClick", function(self)
        ...
        -- wie beim target: sobald der Move-Mode umgeschaltet wird,
    ...
    resetPosButton:SetScript("OnClick", function()
        ...
        -- Layout neu anwenden (Target, nicht target)
    ...
    local debuffsEnableCheck, debuffsSizeSlider, debuffsMaxSlider, debuffsPerRowSlider, debuffsAnchorDD, debuffsXSlider, debuffsYSlider, debuffsGrowDD
        ...
        -- BorderSize je nach Stil aktiv/deaktivieren (für TARGET)
        ...
        leaderIconSizeSlider = CreateSliderWithInput(
            "AI_Target_LeaderIconSizeSlider",
            page,
            "Leader-Icon Größe",
            8, 64, 1,
            leaderIconCheck,
        ...
        leaderIconXSlider = CreateSliderWithInput(
            "AI_Target_LeaderIconXOffsetSlider",
            page,
        ...
        leaderIconYSlider = CreateSliderWithInput(
            "AI_Target_LeaderIconYOffsetSlider",
            page,
        ...
        raidIconXSlider = CreateSliderWithInput(
            "AI_Target_RaidIconXOffsetSlider",
            page,
        ...
        raidIconYSlider = CreateSliderWithInput(
            "AI_Target_RaidIconYOffsetSlider",
            page,
        ...
        debuffsXSlider = CreateSliderWithInput(
            "AI_Target_DebuffsXOffsetSlider",
            page,
            "Debuff X-Offset",
            -400, 400, 1,
            debuffsAnchorDD,
        ...
        debuffsYSlider = CreateSliderWithInput(
            "AI_Target_DebuffsYOffsetSlider",
            page,
            "Debuff Y-Offset",
            -400, 400, 1,
            debuffsXSlider,
        ...
        -------------------------------------------------
        -- target-Controls am Page-Objekt speichern
        -------------------------------------------------
    ...
    -------------------------------------------------
   page.Init = function()
    local entry = GetTargetEntry(moduleKey)
    ...
    elseif entry.enabled == nil then
        ...
        check:SetChecked(entry.enabled and true or false)
            ...
            -- Icon-Defaults (nur für Config-UI, eigentliche Logik ist im target-Modul)
    ...
    check:SetScript("OnClick", function(self)
        ...
        -- Buff-Options (TARGET, nicht target!)
    ...
    if moveButton then
        moveButton:SetScript("OnClick", function(self)
            ...
            if cfg.movable then
                self:SetText("Beenden")
                if AI and AI.modules and AI.modules.target and AI.modules.target.StartMovingMode then
                    AI.modules.target.StartMovingMode()
                end
            else
                self:SetText("Frame bewegen")
                if AI and AI.modules and AI.modules.target and AI.modules.target.StopMovingMode then
                    AI.modules.target.StopMovingMode()
                end
            end
    ...
    if resetPosButton then
        resetPosButton:SetScript("OnClick", function(self)
            AI_Config.modules = AI_Config.modules or {}
            AI_Config.modules.target = AI_Config.modules.target or {}
            local cfg = AI_Config.modules.target

            -- Frame-Position im Modul auf Default zurücksetzen
            if AI and AI.modules and AI.modules.target and AI.modules.target.ResetPosition then
                AI.modules.target.ResetPosition()
            else
            ...
            -- UI neu initialisieren (setzt die Defaults aus GettargetConfig/page.Init)
            if page.Init then
                page.Init()
            end
...
if AI.ConfigUI and AI.ConfigUI.RegisterPage then
    AI.ConfigUI.RegisterPage("target", "Target", 20, BuildTargetConfigPage)
end